...
def SetTryVoltage():
    ...
    #print(f"Tension set to {TryVoltage:.2f} V on generator, OUTPUT = {TryVoltageC:.2f} "
...
def MultiplePulsesProgram ():
    ...
    generator.write('HVON') #start generator
    chConfigEp () # it s ch1Open()  time.sleep(timeSleepRelay)  ch2Open()    
    print ("Charging of generator and condensator... 3sec, at ",TryVoltage,"V")
    ...
    #TryVoltageOUT = float(generator.query('VOUT?'))
    #print ("Pulse start at",TryVoltageOUT,"V")
    #time.sleep(IntervalTime)
    ##TryTime doit être supérieur à timeSleepRelay =0.32e-1s = 32ms
    #ch1Open()
    #end_TryPulse =time.time()
    #time.sleep(timeSleepRelay)#############################
    #ch2Open()
    #print("Interrupteurs ouverts")

    #TryPulseTime = end_TryPulse - start_TryPulse
    #TryPulseTimeDiff =  TryPulseTime - IntervalTime
    #print ("Mesure programme du temps de pulse = ",TryPulseTime,"s, soit une difference de :",TryPulseTimeDiff, "par rapport à la valeur temps set") 
...
def LaunchProgram ():
   ...
   # while True:
   #     voltage = float(generator.query('VSET?')) ##j'ai remplacé VOUT? par VSET? car il bug trop #Get current voltage of the generator
    #    if voltage >= 360:
    #        print(voltage , "V")
    ...
    for LV_pulse_number in range(int(LV_pulse_count)):
        ...
        #time.sleep(timeSleepRelay)#############################
        ch2Open() #Before it was Ch1Open but now the voltage decrease directly (Square wave)
...
def help_callback():
    ...
    text = scrolledtext.ScrolledText(help_window, width=50, height=20)
    text.pack()
...
btCh1 = tk.Button(frame2,text="Transfect",command=ch1Open,bg="orange").pack(side=tk.LEFT)
btCh2 = tk.Button(frame2,text="Monitoring",command=ch1Close,bg="orange").pack(side=tk.LEFT)
btCh3 = tk.Button(frame2,text="Charge",command=ch2Open,bg="orange").pack(side=tk.LEFT)
btCh4 = tk.Button(frame2,text="Disharge",command=ch2Close,bg="orange").pack(side=tk.LEFT)
